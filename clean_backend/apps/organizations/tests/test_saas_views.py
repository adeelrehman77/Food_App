...
class TestSaaSViews:
    ...
    def test_provision_tenant(self, mock_cursor, mock_call_command):
        # Mock DB creation
        ...
        assert Tenant.objects.filter(subdomain='newtenant').exists()
        new_tenant = Tenant.objects.get(subdomain='newtenant')
